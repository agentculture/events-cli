...
events-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. events-cli
...
#   - consumer-identifying prose: `guildmaster` → `events-cli` (NOT
...
If a re-sync would lose a events-cli adaptation, lift the change
...
#     → `which events-cli vendors from guildmaster`
