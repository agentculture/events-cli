...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope events-cli
...
scope:** the default personal scope is **private** (`--scope events-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `events-cli`). Private records are served only to a recall in the same
