...
* from a checkout — ``python -m events_cli``, the documented no-install fallback
...
_MODULE_INVOCATION = "python -m events_cli"

# This package's own __main__.py: events_cli/cli/_prog.py -> events_cli/__main__.py.
# Resolved from __file__ rather than imported, because events_cli/__main__.py
# imports events_cli.cli, which imports this module.
_PACKAGE_MAIN = Path(__file__).resolve().parent.parent / "__main__.py"


def prog_name() -> str:
    """Return the command the caller actually typed."""
    # `python -m events_cli` sets argv[0] to the full path of this package's
