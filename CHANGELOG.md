...
- **Durable subscriptions** (`events sub add|list|show|remove`) — a subscription is an **MQTT persistent session**, not a resident service: `sub add` registers a record *and* establishes a broker-side session (`clean_start=false`, QoS 1, effectively infinite session expiry), then disconnects leaving it live, so the broker's own persistence buffers the backlog while nothing is draining. No always-on control-service container ships. The subscription's identity in the broker **is** its MQTT client id, derived as a pure function of the name (`events-cli-sub-<name>`) so it is stable across processes and collision-free — deliberately not `EventClient`'s per-process-random id, which would mint a fresh empty session on every call. Records carry an `owner` (resolved from `culture.yaml`), which is the same field [#10](https://github.com/agentculture/events-cli/issues/10)'s dynsec identity will occupy, so no migration is needed then. `remove --force` drops a record when the broker is unreachable.
- **`events watch <name> [--since C] [--max 100] [--timeout 30]`** — the bounded cursor drain, the one shape that works identically on a CLI, an MCP tool call and an HTTP request. It replays already-persisted history first (a pure disk read — if that fills `--max`, **no broker session is opened at all**), then drains the broker for the remaining budget, floored at the history page's own cursor so a redelivery can never return an already-replayed event. **There is deliberately no `--follow`**: an unbounded stream hangs an agent turn, and a test asserts the flag does not exist.
- **The history store** (`events_cli/history/`) — an append-only JSONL log per subscription with a fixed-width index sidecar, stdlib-only and dockerless. **The cursor is a store-assigned monotonic sequence**, never an event id: `evt_` ULIDs sort only to millisecond granularity with no intra-millisecond monotonicity. The index entry is written **last** and acts as the commit marker, so a reader takes no lock and still cannot observe a half-written record. Appends dedupe on envelope `id`, which is what makes at-least-once delivery lossless. The store lives beside the per-host stack state (never CWD-relative) and carries a format-version marker from its first write. Building it rather than adopting a sibling is a recorded exception to [#2](https://github.com/agentculture/events-cli/issues/2)'s do-not-build-a-store constraint — see [`docs/decisions/2026-07-24-history-store-evaluation.md`](docs/decisions/2026-07-24-history-store-evaluation.md).
- **`EVENTS_BROKER_HOST` / `EVENTS_BROKER_PORT`** — `events_cli/address.py` is now the single definition of the default broker address; `EventClient` and `BrokerAddress` each previously carried their own `127.0.0.1`/`1883` literals and could drift. A malformed port is a **hard failure**, never a silent fallback to 1883 — that fallback is the precise accident the override exists to prevent.
...
- The generated `mosquitto.conf` now sets **`max_queued_messages 1000`** explicitly. It is mosquitto 2.1.2's own default, restated so the undrained-backlog bound is a documented number rather than an inherited one an image rebuild could change silently. Overflow drops the **newest** message, keeping the oldest backlog — measured on 2026-07-24 (1200 published to an offline session, exactly the oldest 1000 delivered, in order), with the broker's single log notice reachable via `events logs`.
...
- **`events emit` had never worked against a real broker.** `EventClient` connects asynchronously, so `emit` published in the same breath as constructing the client — always while still disconnected — and the message queued inside paho was then discarded by the immediate `close()` (paho's own `loop_stop` docstring is explicit that stopping the loop is not a flush). It failed with `connected: false, reason: "no_conn"` against a perfectly healthy broker. **No unit test could catch this**: every one replaces `EventClient` with a fake, so only the docker-backed integration suite could see it. `emit` now waits, bounded, for the connection, via a new **opt-in `wait` argument** on `publish()`/`publish_event()` that defaults to `0.0` — so the 50 Hz raw lane keeps its O(1) enqueue by construction.
